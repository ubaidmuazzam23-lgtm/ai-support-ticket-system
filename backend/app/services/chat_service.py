...
CNN_CLASS_TO_SEVERITY = {
    "bsod": "critical", "memory_error": "high", "cpu_high": "high",
    "disk_full": "high", "hardware_failure": "critical",
    "dns_error": "high", "network_unreachable": "critical", "vpn_error": "high",
    "ssl_error": "high", "timeout": "medium", "permission_denied": "high",
    "login_failed": "medium", "mfa_error": "medium", "session_expired": "low",
    "app_crash": "high", "update_error": "medium", "install_error": "medium",
    "dependency_error": "medium", "db_connection_error": "critical",
    "db_query_error": "high", "db_timeout": "high",
    "cloud_auth_error": "high", "deployment_error": "critical",
...
CNN_CLASS_LABELS = {
    ...
    "deployment_error": "Deployment / CI-CD Pipeline Error", "container_error": "Container / Docker / Kubernetes Error",
...
- Do not start with "Certainly!" or "Absolutely!" — just get to the point
...
def analyze_screenshot(image_bytes: bytes, session_id: str, user_id: str) -> dict:
    """Save screenshot + run CNN. No Claude Vision."""
    ...
    try:
        ...
        print(f"  🖼  CNN: {cnn_label} ({int(cnn_confidence*100)}% confidence) → {cnn_domain}")

        # Update session domain/severity if CNN is confident
    ...
    if cnn_result and cnn_confidence >= 0.85:
        ...
        display_text = f"Your screenshot looks like it could be a {cnn_label}, though I am not fully certain. Can you describe what happened?"
    else:
...
def process_message(db: Session, user: User, data: ChatMessageRequest) -> ChatMessageResponse:
    ...
    # Add CNN context if screenshot was uploaded
    cnn_context = ""
    if session.get("cnn_result"):
        cr = session["cnn_result"]
        cnn_class = cr.get("error_class", "")
        cnn_conf  = cr.get("confidence", 0.0)
        if cnn_conf >= 0.60 and cnn_class:
            cnn_context = f"\n\nNote: User uploaded a screenshot. CNN detected: {CNN_CLASS_LABELS.get(cnn_class, cnn_class)} ({int(cnn_conf*100)}% confidence). Use this context in your response."

    intent_note = (
        "This is a service request."
        if data.intent == "service_request"
        else f"This is attempt {attempt}. {'Tell the user you recommend raising a support ticket and a human engineer will take over.' if attempt >= 3 else ''}"
    )
    ...
    try:
        ...
        _log_cost(session_id, "Main Response", cost, session["total_cost"],
                  f"attempt={attempt} | domain={session['domain']} | severity={session['severity']}")
...
def _find_best_engineer(db: Session, domain: TicketDomain, user_timezone: str = "UTC") -> Optional[str]:
    ...
    for eng, usr in engineers:
        score = 0
        if domain.value in (eng.domain_expertise or []):
            score += 10
        try:
            user_tz = pytz.timezone(user_timezone)
            eng_tz  = pytz.timezone(eng.timezone)
...
def escalate_to_ticket(db: Session, user: User, data: EscalateRequest) -> EscalateResponse:
    ...
    if ai_msgs:
        ai_diagnosis = ai_msgs[-1][:500]
    if cnn_result.get("error_class"):
        cnn_class = cnn_result["error_class"]
        ai_diagnosis += f"\n\nCNN Screenshot Detection: {CNN_CLASS_LABELS.get(cnn_class, cnn_class)} ({int(cnn_result.get('confidence', 0)*100)}% confidence)"
